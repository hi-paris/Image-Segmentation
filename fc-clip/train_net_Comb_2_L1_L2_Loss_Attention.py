...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def __init__(self, cfg):
        ...
        self.attention_net = nn.Sequential(
            nn.Linear(2, 2),  # Input: 2 losses, Output: 2 attention weights
    ...
    def run_step(self):
        ...
        # Get the total loss for each model
        loss_one = sum(loss_dict_one.values())
        loss_two = sum(loss_dict_model_two.values())
    
        # Compute attention weights based on the losses of both models using softmax
        losses_tensor = torch.tensor([loss_one.item(), loss_two.item()])
        attention_weights = F.softmax(losses_tensor, dim=0)
        
        # Define the dynamically computed attention-based weights for the two models
        alpha1 = attention_weights[0]  # weight for model one based on its loss
        alpha2 = attention_weights[1]  # weight for model two based on its loss
    
        # Combine the individual losses with attention weights
        total_loss = alpha1 * loss_one + alpha2 * loss_two
        ...
        # Use the final total loss for backpropagation
        losses = total_loss
