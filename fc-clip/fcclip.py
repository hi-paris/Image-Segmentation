        ...
        if self.training:
            ...
            for k in list(losses.keys()):
                if k in self.criterion.weight_dict:
                    ...
                    losses.pop(k) 
            return losses, outputs, targets, self.criterion
        else:
            ...
            cls_logits_seen = (
                (in_vocab_cls_results ** (1 - alpha) * out_vocab_cls_probs**alpha).log()# ** (1 - alpha) * out_vocab_cls_probs**alpha  la supprision de out vocab
                * category_overlapping_mask
            )
            cls_logits_unseen = (
                (in_vocab_cls_results ** (1 - beta) * out_vocab_cls_probs**beta ).log()#** (1 - beta) * out_vocab_cls_probs**beta
            ...
            mask_cls_probs = torch.cat([
                cls_results.softmax(-1) * (1.0 - is_void_prob), is_void_prob,# * (1.0 - is_void_prob), is_void_prob, suppression void
                ], dim=-1)
            ...
            return processed_results, mask_cls_results
