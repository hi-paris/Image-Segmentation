...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        # Sum the losses for both models
        loss_one = sum(loss_dict_one.values())
        loss_two = sum(loss_dict_model_two.values())
        
        # Calculate the harmonic mean of the two losses
        harmonic_mean_loss = (2 * loss_one * loss_two) / (loss_one + loss_two + 1e-8)  # add epsilon to avoid division by zero
        ...
        # Use harmonic_mean_loss for final loss computation
        losses = harmonic_mean_loss
