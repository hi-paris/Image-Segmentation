...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        for i in ["pred_logits", 'pred_masks']:
            # Apply softmax to get probabilities
            s1 = F.softmax(logit[i], dim=-1)
            s2 = F.softmax(logit_two[i], dim=-1)
        
            def keep_top_k(tensor, k=10):
                 # Get the indices of the top k values
                 topk_vals, topk_indices = torch.topk(tensor, k, dim=-1)
                 # Create a mask with the same shape as tensor
                 mask = torch.zeros_like(tensor, dtype=torch.bool)
                 # Set the top k indices to True in the mask
                 mask.scatter_(-1, topk_indices, True)
                 # Use the mask to zero out values that are not in the top k
                 return tensor * mask.float()
            

            #s1 = keep_top_k(s1)
            #s2 = keep_top_k(s2)
            if (torch.max(s1) > 0.7) and (torch.max(s2) > 0.7):
                # Select the logit with the higher max value
                new_logit[i] = keep_top_k(torch.max(s1, s2))
            elif torch.max(s1) > 0.7:
                new_logit[i] =keep_top_k(s1)
            elif torch.max(s2) > 0.7:
                new_logit[i] = keep_top_k(s2)
            else:
                # Default to the mean if neither has a high enough max value
                new_logit[i] = keep_top_k(s1) + keep_top_k(s2)
                # Default to the mean if neither has a high enough max value         

        losses_combined = criterion(new_logit, targets)


        for i in losses_combined: 
            loss_dict['com_'+i]= losses_combined[i]
