...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def __init__(self, cfg):
        ...
        self.prev_loss_one = None
        self.prev_loss_two = None
    ...
    def run_step(self):
        ...
        # Initialize previous losses if not set
        if self.prev_loss_one is None:
            self.prev_loss_one = loss_one
        if self.prev_loss_two is None:
            self.prev_loss_two = loss_two

        # Adjust the weights based on the loss progression (difficulty)
        weight_one = self.prev_loss_one / (loss_one + 1e-8)
        weight_two = self.prev_loss_two / (loss_two + 1e-8)
        ...
        total_loss = weight_one * loss_one + weight_two * loss_two
