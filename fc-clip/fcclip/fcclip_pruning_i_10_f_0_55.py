...
class DecoderAdapter(nn.Module):
    ...
    """
    def __init__(self, input_dim, bottleneck_dim, pruning_iterations=10, pruning_fraction=0.55):
        super().__init__()
    ...
    def prune_neurons(self):
        ...
        if self.current_iteration < self.pruning_iterations:
            ...
            # Apply pruning by reducing the bottleneck dimension
            self.bottleneck_dim -= num_to_prune
            self.current_iteration += 1
            
            # Reinitialize the pruned layers with reduced bottleneck dimension
    ...
    def calculate_importance_scores(self):
        """Calculate neuron importance based on MiMi's scoring mechanism"""
        # Use L1 norm or other suitable scoring functions based on MiMi approach
...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.decoder_adapter = DecoderAdapter(input_dim=256, bottleneck_dim=128)  # Start with larger bottleneck_dim
