...
class AttentionDecoderAdapter(nn.Module):
    ...
    def __init__(self, input_dim, bottleneck_dim, num_heads=4):
        ...
        # Layer normalization
        self.norm1 = nn.LayerNorm([bottleneck_dim, 1, 1])
    ...
    def forward(self, x):
        ...
        # Attention block
        # Reshape x to (batch, sequence_length, channels) for multihead attention
        b, c, h, w = x.size()
        x_reshaped = x.view(b, c, -1).permute(0, 2, 1)  # (batch, sequence, channels)
        attn_output, _ = self.attention(x_reshaped, x_reshaped, x_reshaped)  # Self-attention
        x = attn_output.permute(0, 2, 1).view(b, c, h, w)  # Reshape back
...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Replace the DecoderAdapter with the new AttentionDecoderAdapter
        self.decoder_adapter = AttentionDecoderAdapter(input_dim=256, bottleneck_dim=64)
