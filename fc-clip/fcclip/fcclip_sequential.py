...
class DecoderAdapter(nn.Module):
    ...
    def __init__(self, input_dim, bottleneck_dim):
        ...
        self.norm1 = nn.BatchNorm2d(bottleneck_dim)  # Replaced LayerNorm with BatchNorm2d
        self.norm2 = nn.BatchNorm2d(input_dim)       # Replaced LayerNorm with BatchNorm2d
        self.relu = nn.ReLU()
...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Sequential adapters added
        self.adapter1 = DecoderAdapter(input_dim=512, bottleneck_dim=128)  # Add after first convolution block
        self.adapter2 = DecoderAdapter(input_dim=256, bottleneck_dim=64)  # Add after second convolution block

        # Decoder adapter for panoptic segmentation
        self.decoder_adapter = DecoderAdapter(input_dim=256, bottleneck_dim=64)
    ...
    def forward(self, batched_inputs):
        ...
        x = self.adapter1(x)  # Apply first sequential adapter
        x = self.conv2(x)
        x = self.relu(self.bn2(x))
        x = self.adapter2(x)  # Apply second sequential adapter
