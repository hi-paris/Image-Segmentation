...
class DecoderAdapter(nn.Module):
    ...
    def __init__(self, input_dim, bottleneck_dim):
        ...
        self.norm1 = nn.BatchNorm2d(bottleneck_dim)  # Switch to BatchNorm2d
        self.norm2 = nn.BatchNorm2d(input_dim)       # Switch to BatchNorm2d
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.conv2(x)
        x = self.norm2(x)  # Apply batch normalization
        x += identity
        return self.relu(x)
...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        in_channels_res2 = output_shape['res2'].channels
        in_channels_res3 = output_shape['res3'].channels
        in_channels_res4 = output_shape['res4'].channels

        # Define the convolution layers to match the input shape
        self.conv1 = nn.Conv2d(in_channels_res4, 512, kernel_size=3, stride=1, padding=1)
        ...
        # Decoder adapters for res2, res3, and res4
        self.decoder_adapter_res2 = DecoderAdapter(input_dim=in_channels_res2, bottleneck_dim=64)
        self.decoder_adapter_res3 = DecoderAdapter(input_dim=in_channels_res3, bottleneck_dim=64)
        self.decoder_adapter_res4 = DecoderAdapter(input_dim=in_channels_res4, bottleneck_dim=64)
    ...
    def forward(self, batched_inputs):
        ...
        # Apply adapters to res2, res3, and res4
        if isinstance(features, dict):
            if 'res2' in features and 'res3' in features and 'res4' in features:
                res2 = self.decoder_adapter_res2(features['res2'])
                res3 = self.decoder_adapter_res3(features['res3'])
                res4 = self.decoder_adapter_res4(features['res4'])
            else:
                raise KeyError("Expected 'res2', 'res3', and 'res4' in features dictionary, but key not found.")
        else:
            raise KeyError("Expected a dict of features from the backbone.")
    
        # Further processing using res4 (as before)
        x = self.conv1(res4)
