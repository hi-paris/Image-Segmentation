...
class FCCLIP(nn.Module):
    ...
    def forward_GB(self, batched_inputs):
        """
        Args:
        ...
        Returns:
            list[dict]:
                ...
                * "soft_labels":
                    A dict that contains the softmax probabilities and labels for each mask.
        """
        ...
        id_iter=0
        with open("/home/ids/gbrison/FC/id_.txt", 'r') as file:
            for line in file:
                id_iter=int(line.strip())
        ...
        path="/tsi/hi-paris/FCCLIP_results/All_results/INvocab/fcclip_cocopan_r50_1_featurs_outputs_soft_label_test_1/dataloss"


        outputs_path=os.path.join(path, "outputs_"+str(id_iter))
        targets_path=os.path.join(path, "targets_"+str(id_iter))
        features_path=os.path.join(path, "features_"+str(id_iter))
        os.makedirs(path, exist_ok=True) 
        if "instances" in batched_inputs[0]:
                gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
                targets = self.prepare_targets(gt_instances, images)
                new_target=[{i: j[i].to('cpu') for i in j } for j in targets]
        else:
            targets = None
            new_target=None
        ...
        for i in features:
            try:
                new_features[i]=features[i].to('cpu') 
            except:
                new_features[i]=features[i]

        new_outputs={}
        new_outputs['pred_logits']=outputs['pred_logits'].to("cpu")
        new_outputs['pred_masks']=outputs['pred_masks'].to("cpu")
        new_outputs['aux_outputs']=[{j:i[j].to('cpu') for j in i } for i in outputs['aux_outputs']] 

        
        
        with open(outputs_path, 'wb') as file:
            pickle.dump(  new_outputs, file)
        with open(targets_path, 'wb') as file:
            pickle.dump(  new_target, file)
        ...
        if self.training:
            ...
            # Collect soft labels (probabilities and labels)
            soft_labels = {
                "probabilities": mask_cls_probs,
                "labels": mask_cls_results.argmax(dim=-1)
            }
            # upsample masks
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                processed_results.append({
                    "soft_labels": {
                        "probabilities": mask_cls_result.softmax(-1),
                        "labels": mask_cls_result.argmax(dim=-1)
                    }
                })
                ...
                for i in processed_results:
                    try:
                        new_processed_results.append(i.to("cpu"))
                    except:
                        res_={}
                        for j in i:
                            try: 
                                ...
                                for k in i[j]:
                                    try:
                                        result_a[k]=i[j][k].to('cpu') 
                                    except: 
                                        result_a=k
                                res_[j]=result_a
                        new_processed_results.append(res_)
                processed_results_path=os.path.join(path, "processed_results_"+str(id_iter))
                with open(processed_results_path, 'wb') as file:
                    pickle.dump(  new_processed_results, file)
            with open("/home/ids/gbrison/FC/id_.txt", 'w') as file:
                file.write(str(id_iter+1)) 
    ...
    def forward(self, batched_inputs):
        ...
        images = [(x - self.pixel_mean) / self.pixel_std for x in images]
        images = ImageList.from_tensors(images, self.size_divisibility)
        images_names=[x["image_id"] for x in batched_inputs] 
        ...
        features['num_templates'] = num_templates
        outputs = self.sem_seg_head(features)
        outputs["images_names"]=images_names

        

        """path="/tsi/hi-paris/FCCLIP_results/All_results/INvocab/Inference_train_data_new"  

        path_iters=os.path.join(path, "outputs_infer")
        os.makedirs(path_iters, exist_ok=True) 
        number_of_iter=len(os.listdir(path_iters))
        if number_of_iter > 0:

            path_iter=os.path.join(path, "outputs_infer","iter_"+str(number_of_iter-1))
        else:
            path_iter=os.path.join(path, "outputs_infer","iter_"+str(number_of_iter))
            os.makedirs(path_iter, exist_ok=True) 
        number_of_iter=len(os.listdir(path_iter))

        if number_of_iter == 1000000000000:
            path_iter=os.path.join(path, "outputs_infer","iter_"+str(number_of_iter+1))
            os.makedirs(path_iter, exist_ok=True) 


        images_names_path=os.path.join(path_iter, f'images_names_{number_of_iter}')
        outputs_path=os.path.join(path_iter, f'outputs_{number_of_iter}') 
        


        with open(images_names_path, 'wb') as file:
            pickle.dump(  images_names, file) 
        with open(outputs_path, 'wb') as file:
            pickle.dump( {"pred_logits":outputs['pred_logits'],"pred_masks":outputs['pred_masks']}, file)
        ...
        if self.training:
            ...
            processed_results = []
            for mask_cls_result, mask_pred_result, input_per_image, image_size, name_image, cls_results_ in zip(
                mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes, images_names, cls_results
            ):
                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
                processed_results.append({})
                ...
                # panoptic segmentation inference
                if self.panoptic_on:
                    panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result, name_image, cls_results_)
                    processed_results[-1]["panoptic_seg"] = panoptic_r
    ...
    def panoptic_inference(self, mask_cls, mask_pred, name_image, cls_results_):


        scores, labels = F.softmax(mask_cls, dim=-1).max(-1) 
        """

        path="/tsi/hi-paris/FCCLIP_results/All_results/INvocab/fcclip_cocopan_soft_label_calcul_loss"  

        path_iters=os.path.join(path, "Softlabels")
        os.makedirs(path_iters, exist_ok=True) 
        number_of_iter=len(os.listdir(path_iters))
        if number_of_iter > 0:

            path_iter=os.path.join(path, "Softlabels","iter_"+str(number_of_iter-1))
        else:
            path_iter=os.path.join(path, "Softlabels","iter_"+str(number_of_iter))
            os.makedirs(path_iter, exist_ok=True) 
        number_of_iter=len(os.listdir(path_iter))
        if number_of_iter == 1000:
            path_iter=os.path.join(path, "Softlabels","iter_"+str(number_of_iter+1))
            os.makedirs(path_iter, exist_ok=True) 


        mask_cls_path=os.path.join(path_iter, f'mask_cls_{name_image}')
        mask_pred_path=os.path.join(path_iter, f'mask_pred_{name_image}') 
        mask_cls_results_path=os.path.join(path_iter, f'cls_results_{name_image}') 
        


        scores, labels = F.softmax(mask_cls, dim=-1).max(-1) 
        with open(mask_cls_path, 'wb') as file:
            pickle.dump(  mask_cls.to("cpu"), file)
        with open(mask_pred_path, 'wb') as file:
            pickle.dump(  mask_pred.to("cpu"), file) 
        with open(mask_cls_results_path, 'wb') as file:
            pickle.dump(  cls_results_.to("cpu"), file) """
