...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        for i in ["pred_logits", 'pred_masks']:
            ...
            entropy_s1 = calculate_entropy(s1)
            entropy_s2 = calculate_entropy(s2)
            
            # Alpha values based on entropy
            alpha1 = (1 - entropy_s1)
            alpha2 = (1 - entropy_s2)
            
            combined_logit[i] = alpha1 * s1 + alpha2 * s2
    
        # Recalculate combined losses using the combined logit
        combined_loss = criterion(combined_logit, targets)
        
        # Use alpha-weighted loss combining L1 and L2
        total_loss = 0
        for key in combined_loss:
            alpha1 = (1 - calculate_entropy(F.softmax(logit[key], dim=-1)))
            alpha2 = (1 - calculate_entropy(F.softmax(logit_two[key], dim=-1)))
            total_loss += alpha1 * loss_dict_one[key] + alpha2 * loss_dict_model_two[key]
