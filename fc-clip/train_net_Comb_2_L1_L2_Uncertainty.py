...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        # Get losses and logits from both models
        loss_dict_model_two, output_two, targets, criterion = self.model_two(data)
        loss_dict_one, output_one, targets, criterion = self.model(data)
    
        # Extract logits from 'pred_logits' key
        logit_one = output_one["pred_logits"]
        logit_two = output_two["pred_logits"]
    
        # Calculate uncertainty-based weights using the entropy of the softmaxed logits
        softmax_logits_one = F.softmax(logit_one, dim=-1)
        softmax_logits_two = F.softmax(logit_two, dim=-1)
    
        entropy_one = -torch.sum(softmax_logits_one * torch.log(softmax_logits_one + 1e-8), dim=-1)
        entropy_two = -torch.sum(softmax_logits_two * torch.log(softmax_logits_two + 1e-8), dim=-1)
    
        # Normalize the entropies to get weighting factors
        uncertainty_sum = entropy_one + entropy_two
        alpha1 = entropy_two / uncertainty_sum
        alpha2 = entropy_one / uncertainty_sum
    
        # Add the individual losses together with uncertainty-based weights
        total_loss = (alpha1 * sum(loss_dict_one.values()) + 
                      alpha2 * sum(loss_dict_model_two.values())).mean()
    
        # Create a dictionary to track both losses separately
        loss_dict = {}  # Ensure that loss_dict is initialized here
        for i in loss_dict_one: 
            loss_dict['m1_' + i] = loss_dict_one[i]
        for i in loss_dict_model_two: 
            loss_dict['m2_' + i] = loss_dict_model_two[i]
    
        # Final loss computation using uncertainty-based weights
        losses = total_loss
    
        # Backpropagation
        losses.backward()
    
        # Call the metric writing function after loss computation
