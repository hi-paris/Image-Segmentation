...
class KnowledgeDistillationTrainer(DefaultTrainer):
    """
    Extension of the Trainer class adapted to OneFormer with knowledge distillation.
    """
    def __init__(self, cfg):
        super().__init__(cfg)
        self.teacher_model = self.build_teacher_model(cfg)
    ...
    def build_teacher_model(cls, cfg):
        ...
        teacher_cfg.MODEL.WEIGHTS = cfg.MODEL.WEIGHTS_Teacher  # Path to teacher model weights
        model = cls.build_model(teacher_cfg)
        DetectionCheckpointer(model).load(cfg.MODEL.WEIGHTS)
        return model

    def run_step_(self):
        
        assert self.model.training, "[KnowledgeDistillationTrainer] model was changed to eval mode!"
        start = self.iter
        data = next(self._trainer._data_loader_iter)
        
        with torch.no_grad():
            teacher_outputs = self.teacher_model(data)
        logit=self.teacher_model.logma
    ...
    def run_step(self):
        ...
        with torch.no_grad():
            teacher_outputs , teacher_logit= self.teacher_model(data) 
        ...
        loss_dict, student_logit = self.model(data) 

        distillation_loss = self.compute_distillation_loss(student_logit, teacher_logit)
    ...
    def compute_distillation_loss(self, student_outputs, teacher_outputs):
        ...
        """
        # Example: Mean Squared Error between student and teacher logits
...
def main(args):
    ...
    if args.eval_only:
        model = KnowledgeDistillationTrainer.build_model(cfg)
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
            cfg.MODEL.WEIGHTS, resume=args.resume
        )
        res = KnowledgeDistillationTrainer.test(cfg, model)
        if comm.is_main_process():
            verify_results(cfg, res)
        return res
    

    trainer = KnowledgeDistillationTrainer(cfg)
