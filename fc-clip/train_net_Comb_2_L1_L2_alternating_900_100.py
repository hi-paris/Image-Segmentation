...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        # Alternate between training model 1 for the first 900 iterations and model 2 for the last 100
        if self.iter < 900:
            # Train model 1 for the first 900 iterations
            loss_dict_one, logit, targets, criterion = self.model(data)
            loss_dict = {f'm1_{k}': v for k, v in loss_dict_one.items()}
            losses = sum(loss_dict.values())
            losses.backward()
        else:
            # Train model 2 for the last 100 iterations
