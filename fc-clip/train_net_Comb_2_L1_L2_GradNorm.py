...
class CombiningApproachTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        # Create a dictionary to track both losses separately for logging
        loss_dict = {}
        ...
        total_loss = loss_one + loss_two
    
        # Backpropagation with gradient normalization
        total_loss.backward()
    
        # Apply gradient normalization logic
        grad_norm_one = torch.norm(
            torch.stack([torch.norm(p.grad.detach(), 2) for p in self.model.parameters() if p.grad is not None])
        )
        grad_norm_two = torch.norm(
            torch.stack([torch.norm(p.grad.detach(), 2) for p in self.model_two.parameters() if p.grad is not None])
        )
        
        # Normalize gradients
        grad_ratio = grad_norm_one / grad_norm_two
        for p in self.model_two.parameters():
            if p.grad is not None:
                p.grad.data.mul_(grad_ratio)
        
        # Write metrics after backward
